...
import static com.sanri.app.servlet.ClassLoaderServlet.singleClassesDir;
...
//    }
     ...
     * @return classfile 位置
     */
    public void compile(String className, String javaCodes) throws  IOException {
        JavaCompiler compiler = ToolProvider.getSystemJavaCompiler();
        StandardJavaFileManager fileManager = compiler.getStandardFileManager(null,null,null);
        StringObject stringObject = new StringObject(className, javaCodes);
        List<String> options = Arrays.asList("-d", singleClassesDir.getAbsolutePath());
...
//    }
    ...
    public void compile(SimpleJavaBeanBuilder simpleJavaBeanBuilder) throws IOException {
        List<String> build = simpleJavaBeanBuilder.build();
        String javaCodes = StringUtils.join(build, '\n');
        compile(simpleJavaBeanBuilder.getPackageName()+"."+simpleJavaBeanBuilder.getClassName(),javaCodes);
    }
}
